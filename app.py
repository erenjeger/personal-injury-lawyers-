...
with st.sidebar:
    ...
    strict_robots = st.checkbox(
        "Strict robots.txt",
        value=False,
        help="If enabled, an unavailable robots.txt is treated as not allowed.",
    )
...
if run:
    ...
    for city_index, city in enumerate(selected):
        ...
        def cb(done, total, firm_index, firm_total):
            ...
            status.info(
                f"{city['name']}: {done}/{total} attorneys | firm {firm_index}/{firm_total}"
            )
...
if records:
    ...
    st.download_button(
        "⬇️ Download Excel",
        (OUT / "attorney_research.xlsx").read_bytes(),
        "attorney_research.xlsx",
    )
    st.download_button(
        "⬇️ Download CSV",
        (OUT / "attorney_research.csv").read_bytes(),
        "attorney_research.csv",
    )
    st.download_button(
        "⬇️ Download photos ZIP",
        (OUT / "photos.zip").read_bytes(),
        "photos.zip",
    )
